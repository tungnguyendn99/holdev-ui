        ...
        <Card className="flex justify-between items-center px-3 py-2">
          <div>
            <div className="font-medium flex gap-4 items-center">
              <p>{trade.symbol}</p>
              <p className={`${trade.tradeSide === 'BUY' ? 'text-green-500' : 'text-red-500'}`}>
                {trade.tradeSide}
              </p>
              <p className='text-xs text-indigo-400'>{trade.entryPrice} ~ {trade.closePrice}</p>
              {!!trade.images.length && <Images size={16} />}
            </div>
            <p className="flex gap-2 items-center text-xs text-muted-foreground">
              ...
              <span>•</span>
              <span className='font-semibold text-white'>{trade.lots}</span>
              <span>•</span>
          ...
          <div className="text-right">
            ...
            <div
              className={`flex items-center justify-end gap-1 ${trade.rating > 0 ? 'text-yellow-500' : 'text-zinc-400'} text-xs`}
            >
              <Star
                className={`w-3 h-3 ${trade.rating > 0 ? 'fill-yellow-400' : 'fill-zinc-300'}`}
              />{' '}
              {trade.rating || 0}
            </div>
          </div>
